...
public class MiniatureCamera : MonoBehaviour
{
    ...
    private IEnumerator LerpFOV(float startValue, float targetValue, float duration, EasingFunction easing)
    {
        ...
        while (time < duration)
        {
            ...
            _thisCamera.fieldOfView = Mathf.Lerp(startValue, targetValue, AnimationHelper.CalculateEasing(t, easing));
            time += Time.deltaTime;
            yield return null;
        }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(new Rect(200, 300, 400, 100), " start position (miniature camera): " + _startPosition);
        GUI.Label(new Rect(200, 350, 400, 100), " local position (miniature camera): " + transform.localPosition);
    }
}
